...
QHash<Animal*, QVector<Match*>> CompatibilityScorer::calculate_scores() {
	...
	while(animal_i.hasNext()) {
		...
		while(client_i.hasNext()) {
            ...
            match->addRule(QString::fromStdString("The client's ideal animal is "+to_string(want_score)+"compatibile"));
            float personality_score = calculate_client_personality_score(currentAnimal, currentClient);
            match->addRule(QString::fromStdString("The client's personality is "+to_string(personality_score)+"compatibile"));
...
float CompatibilityScorer::calculate_client_want_score(Animal* animal, Client* client) {
...
float CompatibilityScorer::calculate_physical_compatibility(Animal* animal, Animal* idealAnimal) {
	int difference = 0;
...
void CompatibilityScorer::apply_client_situation_heuristics(Animal* animal, Client* client, Match* match) {
	...
	// heuristic no1
    if (client->ownedAnimal && animalAttributes["assert_dominance_tendency"] > (float)7 &&
	...
	// heuristic no3
    if (client->ownsAnimal && animalAttributes["friendliness_animals"] < (float)5) {
    ...
    if (client->hasChildren &&
            (animalAttributes["noise"] < (float)5 || animalAttributes["independence"] > (float)6)) {
